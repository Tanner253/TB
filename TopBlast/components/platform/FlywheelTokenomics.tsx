...
function FlywheelTree({ compact = false }: { compact?: boolean }) {
  return (
    <div
      className={`rounded-xl border border-rh-green/20 bg-card/70 font-mono text-sm leading-relaxed ${
        compact ? 'p-4' : 'p-5'
      }`}
    >
      <div className="text-ink-2">{FLYWHEEL_TREE.root}</div>
      <div className="mt-2 pl-3 border-l-2 border-rh-green/30 space-y-2">
        <div>
          <span className="text-sol-purple">├─ </span>
          <span className="text-ink-2">{FLYWHEEL_TREE.buyback}</span>
        ...
        <div>
          <span className="text-sol-purple">└─ </span>
          <span className="text-ink-2">{FLYWHEEL_TREE.ops}</span>
        </div>
      </div>
      <p className="mt-4 text-xs text-ink-3 font-sans leading-normal">{FLYWHEEL_TREE.burnNote}</p>
    </div>
  )
}
  ...
  return (
    <section className={compact ? '' : 'glass-panel rounded-2xl p-8 border-rh-green/20'}>
      <div className={compact ? '' : 'max-w-3xl mx-auto'}>
        <h2 className={`font-bold text-sol-purple ${compact ? 'text-xl mb-3' : 'text-2xl mb-4 text-center'}`}>
          Platform fee flywheel
        </h2>
        <p className={`text-ink-2 ${compact ? 'text-sm mb-4' : 'text-center mb-6'}`}>{FLYWHEEL_INTRO}</p>
