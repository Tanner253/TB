...
export function LaunchSetupChecklist({ className = '' }: { className?: string }) {
  return (
    <section className={`rounded-2xl border border-rh-green/20 bg-rh-green/5 p-6 ${className}`}>
      <h2 className="text-lg font-bold text-sol-purple mb-4">{HOW_TO_RUN_LISTING.title}</h2>
      <ol className="space-y-4">
        {HOW_TO_RUN_LISTING.steps.map(step => (
          <li key={step.n} className="flex gap-3">
            <span className="flex h-8 w-8 shrink-0 items-center justify-center rounded-full bg-rh-green/20 text-sm font-bold text-sol-purple">
              {step.n}
            </span>
            <div>
...
export function LaunchAfterSubmitFlow({ className = '' }: { className?: string }) {
  return (
    <section className={`rounded-2xl border border-line bg-card/70 p-6 ${className}`}>
      <h2 className="text-lg font-bold mb-4">{LAUNCH_FLOW.title}</h2>
      <ul className="space-y-4">
        {LAUNCH_FLOW.steps.map((step, i) => (
          <li key={step.title} className="flex gap-3">
            <span className="text-ink-3 font-mono text-sm pt-0.5">{i + 1}.</span>
            <div>
