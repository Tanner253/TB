...
export function CatalogListRow({ tenant }: { tenant: PublicTenantSummary }) {
  ...
  return (
    <Link
      ...
      className={`group grid grid-cols-[minmax(0,1.4fr)_minmax(0,1fr)_auto_auto] gap-3 md:gap-4 items-center px-4 py-3 border-b border-white/[0.06] hover:bg-white/[0.03] transition-colors ${
    ...
    >
      ...
      <span className="text-xs text-gray-400 whitespace-nowrap">{payoutLabel(tenant)}</span>
